\begin{frame}{Collision}
    %\Slide->1    Latex->2    Samee->3    Tamim->0
    \begin{tikzpicture}[remember picture, overlay, shift={(2,0)}]
    
        \draw[fill=red!50,visible on=<6>] (0,0) rectangle (2,2);
        
        \draw (0,0) rectangle (2,2);
        \draw (2,0) rectangle (4,2);
        \draw (4,0) rectangle (6,2);
        \draw (6,0) rectangle (8,2);

        \node[visible on=<2->] (a) at (1,-2) {\texttt{Tamim}};
        \node[visible on=<2->] (c) at (5,-2) {\texttt{Slide}};
        \node[visible on=<2->] (b) at (3,-2) {\texttt{Overleaf}};
      

        \node[color=red, visible on=<3->] (e) at (3,3) {\texttt{Hash function: Sum of ASCII mod 4}};

        \draw[-stealth, visible on=<4->] (1,-1.75) -- (1,1);
        \draw[-stealth, visible on=<5->] (5,-1.75) -- (3,1);
        \draw[-stealth, visible on=<6->] (3,-1.75) -- (1,1);
     

        \node[color=blue!50, visible on=<4>] at (3.25,-3) {\texttt{Hash value: (84+65+77+73+77)mod4 = 0}};
        \node[color=blue!50, visible on=<5>] at (3.25,-3) {\texttt{Hash value: (83+76+73+68+69)mod4 = 1}};
        \node[color=blue!50, visible on=<6>] at (3.25,-3) {\texttt{Hash value: (79+86+69+82+76+69+65+70)mod4 = 0}};
        
    \end{tikzpicture}
\end{frame}
